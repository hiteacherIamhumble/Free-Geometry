...
# =============================================================================
# DA3 Giant: Strong LoRA Distillation — feature-only distill + LoRA
#
# This script can:
#   - train per-dataset LoRA adapters
#   - benchmark both LoRA and baseline across multiple "views" settings
#
# NOTE: Despite the historical filename/comments, this runs all datasets in
# ALL_DATASETS (not scannetpp-only) unless you choose a single-dataset command.
# =============================================================================
...
OUTPUT_DIR=./checkpoints/da3_strong_lora
BENCHMARK_ROOT=./workspace/da3_strong_lora_v2
...
get_training_config() {
    local DATASET=$1
    case "${DATASET}" in
        scannetpp) echo "5 3 40 44" ;;
        hiroom)    echo "3 3 42 44" ;;
        7scenes)   echo "10 3 40 49" ;;
        eth3d)     echo "5 3 40 44" ;;
        *)
...
train_single_dataset() {
    ...
    echo ""
    echo "============================================================"
    echo "Training DA3 on ${DATASET} — STRONG LoRA DISTILL"
    echo "  Samples/scene: ${SAMPLES}, Seeds: ${SEEDS}, Epochs: ${DS_EPOCHS}"
    echo "  LoRA rank=${LORA_RANK}, LR=${LR}, cosine scheduler"
    echo "============================================================"
...
run_lora_benchmark() {
    echo ""
    echo "============================================================"
    echo "Benchmarking DA3 Strong LoRA — all epochs"
    echo "  Datasets: ${ALL_DATASETS}"
    echo "  Max frames: ${MAX_FRAMES_LIST}"
    echo "============================================================"

    for DATASET in ${ALL_DATASETS}; do
        # Discover all saved LoRA epochs for this dataset (epoch_<N>_lora.pt).
        # Avoid `ls` here because a missing glob can trip `set -e`.
        shopt -s nullglob
...
usage() {
    ...
    echo "  MAX_FRAMES_LIST='${MAX_FRAMES_LIST}'"
}
