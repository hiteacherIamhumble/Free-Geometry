...
# =============================================================================
# VGGT Experiment V2: Train 4 models, Benchmark with 4v/8v/maxframe settings
# =============================================================================
#
# This script:
# 1. Trains 4 distilled models (8v teacher → 4v student), one per dataset
# 2. Benchmarks baseline VGGT with 3 settings: 4v, 8v, maxframe (100)
# 3. Benchmarks all 4 LoRA models with same 3 settings on all 4 datasets
# 4. Generates comparison tables for each setting
#
# Usage:
#   ./scripts/run_vggt_experiment_v2.sh [train|benchmark_base|benchmark_lora|compare|all]
#
# =============================================================================

# === Configuration ===
MODEL_NAME=facebook/vggt-1b
OUTPUT_DIR=./checkpoints/vggt_experiment_v2
BENCHMARK_ROOT=./workspace/vggt_experiment_v2
...
get_benchmark_params() {
    ...
    case "${SETTING}" in
        ...
        8v)
            ...
            ;;
        maxframe)
            echo "100 0"  # max_frames=100, eval_frames=0 (use all)
            ;;
    esac
}
...
benchmark_base_setting() {
    ...
    echo "  [Base ${SETTING}] all datasets, seed=${SEED} (max=${MAX_FRAMES}, eval=${EVAL_FRAMES})"
    ...
    local CMD="python ./scripts/benchmark_lora_vggt.py \
        ...
        --seed ${SEED} \
        --image_size 518 \
        --work_dir ${BENCHMARK_ROOT}/base_${SETTING}/seed${SEED}"
...
benchmark_lora_setting() {
    ...
    echo "  [LoRA-${TRAIN_DATASET} ${SETTING}] all datasets, seed=${SEED}"
...
run_baseline_benchmark() {
    ...
    echo "  Total runs: 3 settings × 3 seeds = 9"
    echo "============================================================"
...
run_lora_benchmark() {
    echo ""
    echo "============================================================"
    echo "STEP 3: Benchmarking 4 LoRA models"
    echo "  Settings: 4v, 8v, maxframe"
    echo "  Train datasets: ${ALL_DATASETS}"
    echo "  Eval datasets: ${ALL_DATASETS}"
    echo "  Seeds: ${BENCHMARK_SEEDS}"
...
run_comparison() {
    ...
    echo "  Settings: 4v, 8v, maxframe"
    echo "  Train datasets: ${ALL_DATASETS}"
    echo "  Total comparisons: 3 settings × 4 train_datasets = 12"
    echo "============================================================"
    ...
    for SETTING in 4v 8v maxframe; do
        ...
        for TRAIN_DATASET in ${ALL_DATASETS}; do
            echo "  Comparing LoRA-${TRAIN_DATASET} vs baseline (${SETTING})..."

            python ./scripts/compare_vggt_results.py \
                --base_root "${BENCHMARK_ROOT}/base_${SETTING}" \
                --lora_root "${BENCHMARK_ROOT}/lora_${SETTING}/${TRAIN_DATASET}" \
                --output_dir "${BENCHMARK_ROOT}/comparison_${SETTING}/${TRAIN_DATASET}" \
...
usage() {
    ...
    echo "  benchmark_base  - Benchmark baseline VGGT (9 runs)"
...
echo ""
echo "============================================================"
echo "=== VGGT Experiment V2 Complete ==="
echo "============================================================"
echo ""
