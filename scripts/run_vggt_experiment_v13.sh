...
# =============================================================================
# VGGT Experiment V13: 16v→8v with Cross-Frame RKD
# =============================================================================
#
# This experiment:
# - 16v→8v distillation with random sampling (same as V8)
# - Loss: all-token softmax KL + cosine + cross-frame RKD (from V11/V12)
# - scannetpp only, 5 samples/scene, seeds 30-34
# - Benchmark: maxframe only, seed 43
#
# Training config:
#   scannetpp: 5 samples/scene, seeds 30 31 32 33 34
#
# Usage:
#   ./scripts/run_vggt_experiment_v13.sh [train|benchmark_lora|all]
#
# =============================================================================

# === Configuration ===
MODEL_NAME=facebook/vggt-1b
OUTPUT_DIR=./checkpoints/vggt_experiment_v13
BENCHMARK_ROOT=./workspace/vggt_experiment_v13
DATA_BASE=/home/22097845d/Depth-Anything-3/workspace/benchmark_dataset

# Training settings (16v→8v from V8)
EPOCHS=2
LR=1e-4
LORA_RANK=16
LORA_ALPHA=16
...
RKD_WEIGHT=2.0
RKD_TOPK=4
RKD_NUM_REF_SAMPLES=128
RKD_NUM_SHARED_SAMPLES=128
RKD_ANGLE1_WEIGHT=1.0
RKD_ANGLE2_WEIGHT=1.0
RKD_ANGLE3_WEIGHT=1.0

# Benchmark settings
BENCHMARK_SEEDS="42 44"
...
train_scannetpp() {
    ...
    local SAMPLES=5
    local SEEDS="$(seq -s ' ' 30 34)"

    echo ""
    echo "============================================================"
    echo "Training on ${DATASET} (16v→8v + RKD)"
...
run_training() {
    echo ""
    echo "============================================================"
    echo "Training: 16v→8v + RKD on scannetpp"
    echo "============================================================"
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  train           - Train 16v→8v LoRA with RKD on scannetpp"
...
echo ""
echo "============================================================"
echo "=== VGGT Experiment V13 Complete ==="
echo "============================================================"
echo ""
echo "Experiment: 16v→8v + KL + cosine + RKD on scannetpp"
